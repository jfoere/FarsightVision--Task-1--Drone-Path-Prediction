...
python -m reference_path video1.MP4 --align 8 13
python -m unittest discover -s tests
```

`debug_ui` shows intermediate decisions while the video plays. `reference_path` optionally uses an independent SIFT/homography pipeline and embedded DJI GPS metadata to create an estimated reference for comparison.
